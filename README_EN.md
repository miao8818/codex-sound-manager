...
- Dragging uses Tauri's native Windows window-drag path instead of WebView coordinate frames and an IPC queue, so the orb follows the system cursor directly without toggling sound by mistake.
...
| NSIS installer | `target\release\bundle\release\CodexSoundManager_1.3.5_x64-setup.exe` |
| Portable ZIP | `target\release\bundle\portable\CodexSoundManager_1.3.5_x64-portable.zip` |
