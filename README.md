...
- 拖动使用按下点到当前位置的 DPI 缩放总位移，只保留最新待处理帧，起步不跳动，长距离移动也不会逐渐落后；超过操作阈值后不会误触提示音开关。
...
| NSIS 安装包 | `target\release\bundle\release\CodexSoundManager_1.3.4_x64-setup.exe` |
| 便携 ZIP | `target\release\bundle\portable\CodexSoundManager_1.3.4_x64-portable.zip` |
